...
// A-Z
export const alphaRange: string[] = [
  ...R.map((item: number) => String.fromCharCode(item).trim(), R.range(65, 65 + 26)),
  'AA',
  'AB',
  'AC',
  'AD',
...
export const handlePaper: (paper: IPaper[]) => IPaper[] = paper =>
  paper.map(item => {
    if (typeof item.data === 'string') {
      ...
      //   item.dept = R.reject((a: string) => a.length === 0)(item.dept);
      //   item.desc = item.desc.split('\n').map(item => item.trim());
      //   item.desc = R.reject((a: string) => a.length === 0)(item.desc);
      // }
    }
...
export const parseAnswer = (
  ...
  return (
    <div style={showErr && curAnswer === '' ? { color: '#e23' } : {}}>
      {title}
      {maxLength && `(${state[key].length}/${maxLength})`}
      {/* <span> ( {curAnswer} ) </span> */}
    </div>
  );
};
